...
export default function BookingsPage() {
  ...
  // Filter bookings using debounced search and status filter
          ...
          {filtered.length > pageSize && (
            <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50/40">
              <div className="text-xs text-slate-500">
                {language === "bm"
                  ? `Menunjukkan ${(currentPage - 1) * pageSize + 1} - ${Math.min(currentPage * pageSize, filtered.length)} daripada ${filtered.length} rekod`
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3 py-1.5 rounded-xl border border-slate-200 bg-white text-xs font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-40 disabled:cursor-not-allowed transition flex items-center gap-1"
                >
                  <ChevronLeft className="w-3.5 h-3.5" />
                  {language === "bm" ? "Sebelumnya" : "Previous"}
                </button>
                <span className="px-3 py-1 text-xs font-bold text-indigo-600 bg-indigo-50 border border-indigo-100 rounded-xl">
